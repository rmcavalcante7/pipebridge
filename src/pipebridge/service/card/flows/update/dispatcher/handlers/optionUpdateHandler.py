...
class OptionUpdateHandler(BaseCardFieldUpdateHandler):
    """
    Resolve single-option field updates.

    This handler is shared by field types that semantically accept a single
    string option, such as ``select``, ``radio_*`` and ``label_select``.
    """
    ...
    def resolve(
        ...
        """
        Resolve single-option updates into a single selected string value.
        ...
        :raises ValidationError:
            When input is not a string option
        """
        class_name, method_name = getExceptionContext(self)
        if not isinstance(input_value, str):
            raise ValidationError(
                message="Single-option fields require a string value",
