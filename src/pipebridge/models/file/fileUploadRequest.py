...
class FileUploadRequest:
    ...
    def __init__(
        ...
        """
        Initialize a validated file upload request.
        ...
        if not file_name or not isinstance(file_name, str):
            raise ValidationError(
                "file_name must be a non-empty string", class_name, method_name
            )

        if not isinstance(file_bytes, bytes):
            raise ValidationError(
                "file_bytes must be of type bytes", class_name, method_name
            )

        if not card_id or not isinstance(card_id, str):
            raise ValidationError(
                "card_id must be a non-empty string", class_name, method_name
            )

        if not field_id or not isinstance(field_id, str):
            raise ValidationError(
                "field_id must be a non-empty string", class_name, method_name
            )

        if not organization_id or not isinstance(organization_id, str):
            raise ValidationError(
                "organization_id must be a non-empty string", class_name, method_name
            )

        if not isinstance(replace_files, bool):
            raise ValidationError(
                "replace_files must be a boolean", class_name, method_name
            )

        if expected_phase_id is not None and not isinstance(expected_phase_id, str):
            raise ValidationError(
                "expected_phase_id must be a non-empty string", class_name, method_name
            )
    ...
    def fromFile(
        ...
        if not file_path or not isinstance(file_path, str):
            raise ValidationError(
                "file_path must be a non-empty string", class_name, method_name
            )
