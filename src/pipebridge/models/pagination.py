...
from pipebridge.exceptions import ParsingError
...
class PageInfo(BaseModel):
    ...
    def __init__(self, hasNextPage: bool, endCursor: Optional[str] = None) -> None:
        """
        Initialize PageInfo.
    ...
    def fromDict(cls, data: Dict[str, Any]) -> "PageInfo":
        ...
        return cls(
            hasNextPage=data.get("hasNextPage", False), endCursor=data.get("endCursor")
        )
...
class Edge(Generic[T]):
    ...
    @classmethod
    def fromDict(
        cls, data: Dict[str, Any], node_parser: Callable[[Any], T]
    ) -> "Edge[T]":
        """
...
class PaginatedResponse(Generic[T]):
    ...
    def __init__(self, edges: List[Edge[T]], page_info: PageInfo) -> None:
        """
...
if __name__ == "__main__":
    ...
    def mockParser(data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Echo example node data for the standalone pagination usage sample.

        :param data: dict[str, Any] = Example node payload

        :return: dict[str, Any] = Unmodified node payload
        """
        return data
