...
class PhaseHistory(BaseModel):
    ...
    def __init__(
        self, phase: Phase, first_time_in: Optional[str], last_time_in: Optional[str]
    ) -> None:
        """
        Initialize a phase history entry.

        :param phase: Phase = Phase reached by the card
        :param first_time_in: str | None = Timestamp for first entry into phase
        :param last_time_in: str | None = Timestamp for last exit from phase
        """
