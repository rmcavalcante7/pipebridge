...
from pipefy.dispatcher.field.fieldValidator import FieldValidator
...
class CardService:
   ...
   Service responsible for card operations in Pipefy.
    ...
    def updateFields(
        ...
        # --------------------------------------------------------
        # Load card and phase context
        # --------------------------------------------------------
        card: Card = self.getCardModel(card_id)

        current_phase_id: str = card.current_phase
        ...
        phase = self._phase_service.getPhase(current_phase_id)
        ...
        for field_id, value in fields.items():
            ...
            phase_field = phase.fields_map.get(field_id)
            ...
            validator.validate(
                ...
                expected_phase_id=current_phase_id,
                field_id=field_id,
                phase_field=phase_field,
                value=value
            )

            field_type: str = phase_field.type

            # ----------------------------------------------------
            # Build mutation (attachment vs default)
            # ----------------------------------------------------
            if field_type == "attachment":
                mutation = self._buildAttachmentMutation(
                    card_id=card_id,
                    field_uuid=phase_field.uuid,
                    urls=value
                )
            else:
                mutation = self._buildDefaultMutation(
                    card_id=card_id,
                    field_id=field_id,
                    value=value
                )
            ...
            # Execute request
            # ----------------------------------------------------
            response = self._client.sendRequest(mutation)
